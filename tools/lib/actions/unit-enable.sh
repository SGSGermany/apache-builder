...
action_exec() {
    ...
    # we thus just uncomment the '[Install]' section inside the Quadlet Systemd unit file
    ...
    if grep -q -Fx '[Install]' "$UNIT_PATH_QUADLET"; then
        # unit is already enabled, nothing to do
        return 0
    elif ! grep -q -Fx '#[Install]' "$UNIT_PATH_QUADLET"; then
        # unit can't be enabled, bail with error
    ...
    cmd gawk -i inplace '/\[.+\]/ { found=0 } /#\[Install\]/ { found=1 }
        found && /^#\S/ { print substr($0, 2) } !found || !/^#\S/ { print }' "$UNIT_PATH_QUADLET"
